...
*   **MLX Inference Engine**: Uses `MLXInferenceEngine` singleton (`mlx_engine.py`) to serve **Gemma 4 26B A4B MoE** and **Nemotron-Cascade-2 30B** via `mlx_lm` and `mlx_vlm` directly on Apple Silicon GPU, with in-memory QLoRA adapter hot-swapping for regime-aware model customization.
*   **SwarmOrchestrator**: Coordinates the agentic swarm (Quant, Forecast, Research, Risk, Whale, Goal) via a non-blocking 2-stage streaming pipeline.
...
| **The Reasoner** | **GPU (Metal/MLX)** | Direct MLX inference via `mlx_lm`/`mlx_vlm`, QLoRA adapter fine-tuning and hot-swapping. | 60% memory rule (Weights + KV ≤ 28GB) |
...
- **Secure Sandboxing**: Code generated by mathematical agents runs via `SafePythonExecutor`.
...
*Verified for Apple Silicon M4 Pro/Max/Ultra (June 2026)*
