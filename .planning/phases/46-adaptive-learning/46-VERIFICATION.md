...
- **Status:** ✅ VERIFIED
- **Evidence:** Executed QLoRA training on the native model target `mlx-community/gemma-4-E4B-it-qat-4bit` using `mlx_vlm.lora` with prompt masking enabled. The training ran successfully on Metal GPU for 20 iterations, dropping loss to 0.874 and saving `adapters/high_vol_bull/adapters.safetensors`. Memory usage peaked at 12.62 GB, well within the 28GB (60%) VRAM budget.
...
- **Evidence:**
  ...
  - `scripts/prepare_training_data.py` calculated Rolling Volatility, RSI, ATR, and CVD indicators, generated Buy/Hold/Sell labels from forward-looking returns, and exported CSV and JSONL datasets in the required `question`/`answer` format. Verified zero nulls and correct formatting.

### Must-Have 3: Dynamic LoRA adapter switching based on detected market regimes
- **Status:** ✅ VERIFIED
- **Evidence:**
  - Implemented `switch_adapter` inside `MLXInferenceEngine` (`backend/mlx_engine.py`) supporting both `mlx_lm` and `mlx_vlm` model formats.
  - Executed `scripts/benchmark_adapter_hotswap.py` for 100 swaps on `mlx-community/gemma-4-E4B-it-qat-4bit`: average swap latency is **10.47 ms** (budget: <50ms), and RSS memory change is **+0.0065%** (virtually zero, completely avoiding the 2x model RAM spike).
