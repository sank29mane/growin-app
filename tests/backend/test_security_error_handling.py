...
def test_analyze_error_handling_sanitization():
    ...
    with TestClient(app) as client:
        ...
        with patch("agents.coordinator_agent.CoordinatorAgent") as mock_coordinator,              patch("agents.orchestrator_agent.OrchestratorAgent", create=True) as mock_orchestrator:
            mock_instance = MagicMock()
            mock_instance.process_query.side_effect = Exception("SENSITIVE_INFO_LEAKED_ANALYZE")
            mock_coordinator.return_value = mock_instance

            mock_orch_instance = MagicMock()
            mock_orch_instance.run.side_effect = Exception("SENSITIVE_INFO_LEAKED_ANALYZE")
            mock_orchestrator.return_value = mock_orch_instance
