...
class CurrencyNormalizer:
    ...
    **Display Format**: Always show base currency (£ for GBP, $ for USD)
    ...
    def normalize_position(position: Dict[str, Any], metadata_cache: Optional[Dict] = None) -> Position:
        ...
        """
        ticker = position.get('ticker') if position.get('ticker') is not None else position.get('symbol', '')
        ...
        # Extract raw values
        raw_current = position.get('currentPrice') if position.get('currentPrice') is not None else position.get('current_price', 0)
        raw_avg = position.get('averagePrice', 0)
        qty = position.get('quantity') if position.get('quantity') is not None else position.get('qty', 0)
