    ...
    async def optimize_weights(
        ...
        try:
            ...
            if isinstance(self.model, NeuralJMCE) and HAS_MLX:
                ...
                # Fallback to CPU/Numpy if no acceleration target is available
                logger.warning("No usable NPU acceleration found. Falling back to Numpy/CPU.")
                mu = np.mean(returns_history, axis=0)
                if returns_history.shape[1] == 1:
                    sigma = np.atleast_2d(np.var(returns_history, axis=0))
                else:
