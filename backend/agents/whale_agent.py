...
class WhaleAgent(BaseAgent):
    ...
    def __init__(self, config: AgentConfig = None):
        ...
        self.whale_threshold_usd = (
            50000.0  # Lowered from $250k for paper/IEX data density
        )
    ...
    async def analyze(self, context: Dict[str, Any]) -> AgentResponse:
        ...
        try:
            ...
            if not trades:
                logger.info(
                    f"WhaleAgent: No trades found for {ticker}. Attempting Volume Anomaly Detection..."
                )
            ...
            threshold = create_decimal(self.whale_threshold_usd)
            for t in trades:
                    ...
                    large_trades.append(
                        {
                            ...
                            "value_usd": float(
                                value
                            ),  # Frontend property is valueUsd but mapping handles it
                            "timestamp": str(t["t"]),
            ...
            if len(large_trades) > 0:
                ...
                whale_avg_price = sum(
                    create_decimal(w["price"]) for w in large_trades
                ) / len(large_trades)
    ...
    async def _analyze_via_volume_anomaly(self, ticker: str) -> AgentResponse:
        ...
        try:
            ...
            avg_vol = (
                sum(prev_vols) / len(prev_vols) if prev_vols else create_decimal(1)
            )
