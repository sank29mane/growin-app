...
"""
                ...
                if any(x in error_msg for x in ["not found", "ticker", "delisted", "no data", "404"]):
                    ...
                    if "ticker" in context:
                        ...
                        if new_ticker and new_ticker != ticker:
                            ...
                            try:
                                retry_result = await run_with_timeout(agent.execute(new_context), 15.0)
                                ...
                                logger.warning(f"Specialist {agent.config.name} timed out during Tier 2 retry")
    ...
    async def _resolve_ticker_via_search(self, term: str) -> Optional[str]:
        ...
        try:
            ...
            if self.mcp_client:
                try:
                    logger.info(f"Coordinator Tier 2: Searching for correct ticker matching '{term}' via MCP")
                ...
                logger.info("Coordinator Tier 2: MCP not available, falling back to local resolver.")
                resolver = TickerResolver()
                search_result = await resolver.search(term)
            ...
            if search_result and isinstance(search_result, list) and len(search_result) > 0:
                ...
                # 3. Return best match if high enough confidence
                # Threshold for similarity (arbitrary 0.6)
                if best_match and highest_score > 0.6:
                    found_ticker = best_match["ticker"]
                    
                    # Apply SOTA normalization
                    resolver = TickerResolver()
                    normalized = resolver.normalize(found_ticker)
            ...
            logger.warning(f"Ticker search resolution failed: {e}")
            ...
            if match:
                ...
                if python_code:
                    ...
                    if not self.mcp_client:
                        logger.warning("Coordinator: MCP client not available for Docker fix")
                        return None
                    ...
                    try:
                        ...
                        if result and result.content:
                            ...
                            except (SyntaxError, ValueError) as ae:
                                logger.warning(f"Failed to parse Docker output: {ae}")
                                return None

                            if exec_res.get("exit_code") == 0:
                                stdout = exec_res.get("stdout", "").strip()
                                if not stdout:
                                    logger.warning("Docker execution returned empty stdout")
                                    return None
                                try:
                                    new_context = json.loads(stdout)
                                except json.JSONDecodeError as jde:
                                    logger.warning(f"Failed to parse Docker stdout as JSON: {jde}. Stdout: {stdout}")
                                    return None
                                
                                logger.info(f"Coordinator retry for {agent.config.name} with fixed context: {new_context}")
                                # Wrap retry in timeout as well
                                return await run_with_timeout(agent.execute(new_context), 15.0)
                            else:
