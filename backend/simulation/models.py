...
class MarketImpactModel:
    ...
    def calculate_slippage(
        ...
        if len(sizes) == 0:
            ...
            sizes_clipped = np.maximum(sizes, 0)
            cum_sizes = np.cumsum(sizes_clipped)
            idx = np.searchsorted(cum_sizes, abs_size)

            if idx < len(sizes_clipped):
                if idx > 0:
                    weighted_price_sum = np.dot(sizes_clipped[:idx], prices[:idx])
                    rem = abs_size - cum_sizes[idx-1]
                    weighted_price_sum += rem * prices[idx]
                else:
                    weighted_price_sum = abs_size * prices[0]
                accumulated_qty = float(abs_size)
            else:
                weighted_price_sum = np.dot(sizes_clipped, prices)
