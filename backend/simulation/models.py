...
class MarketImpactModel:
    ...
    def calculate_slippage(
        ...
        # Vectorized implementation replacing the for loop
        valid_mask = sizes > 0
        ...
        if len(valid_sizes) == 0:
            ...
            # Find how many full levels are consumed
            idx = np.searchsorted(cum_sizes, abs_size)

            if idx == 0:
                # First level satisfies the whole order
                weighted_price_sum = abs_size * valid_prices[0]
                accumulated_qty = abs_size
            elif idx < len(cum_sizes):
                # Consumes idx full levels and part of the next
                weighted_price_sum = np.sum(valid_prices[:idx] * valid_sizes[:idx])
                rem_size = abs_size - cum_sizes[idx - 1]
                weighted_price_sum += rem_size * valid_prices[idx]
                accumulated_qty = abs_size
            else:
                # Consumes all available levels, apply penalty to remaining
