    ...
    try:
        ...
        for bar_data in bar_list:
            try:
                ...
                close_val = float(CurrencyNormalizer.normalize_price(bar_data.get("c") if bar_data.get("c") is not None else bar_data.get("close", 0), ticker))
                high_val = float(CurrencyNormalizer.normalize_price(bar_data.get("h") if bar_data.get("h") is not None else bar_data.get("high", 0), ticker))
                low_val = float(CurrencyNormalizer.normalize_price(bar_data.get("l") if bar_data.get("l") is not None else bar_data.get("low", 0), ticker))
                open_val = float(CurrencyNormalizer.normalize_price(bar_data.get("o") if bar_data.get("o") is not None else bar_data.get("open", 0), ticker))
                volume_val = int(bar_data.get("v") if bar_data.get("v") is not None else bar_data.get("volume", 0))
...
async def get_yfinance_chart_data(ticker: str, timeframe: str, limit: int, cache_key: str):
    ...
    try:
        ...
        for p in raw_points:
            p_norm = p.copy()
            close_val = float(CurrencyNormalizer.normalize_price(p.get("close") if p.get("close") is not None else p.get("c", 0), ticker))
            p_norm["close"] = close_val
            p_norm["c"] = close_val
            p_norm["high"] = float(CurrencyNormalizer.normalize_price(p.get("high") if p.get("high") is not None else p.get("h", 0), ticker))
            p_norm["h"] = p_norm["high"]
            p_norm["low"] = float(CurrencyNormalizer.normalize_price(p.get("low") if p.get("low") is not None else p.get("l", 0), ticker))
            p_norm["l"] = p_norm["low"]
            p_norm["open"] = float(CurrencyNormalizer.normalize_price(p.get("open") if p.get("open") is not None else p.get("o", 0), ticker))
