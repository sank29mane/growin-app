...
## 2026-04-22 - SwiftUI .buttonStyle(.plain) Accessibility Context
**Learning:** Consistently found that developers use `.buttonStyle(.plain)` to suppress native visual button chrome, which inadvertently removes standard accessibility boundaries and traits, rendering the buttons silent or functionally meaningless to VoiceOver users.
**Action:** When finding missing `.accessibilityLabel` tags on custom components using `.buttonStyle(.plain)`, consistently apply `.accessibilityAddTraits(.isButton)`. For dynamically active buttons (e.g. Account Pickers), conditionally append `.isSelected` alongside `.isButton`.
