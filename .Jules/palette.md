...
## 2026-04-17 - SwiftUI Custom Picker Button Accessibility
**Learning:** When using custom horizontal pickers (like ) in SwiftUI, applying `.buttonStyle(.plain)` completely removes their interactive context. While `.accessibilityLabel` is necessary, dynamically including the `[.isSelected, .isButton]` traits is critical for VoiceOver users to understand both the purpose (it's a button) and its current state (whether it is active/selected).
**Learning:** When using custom horizontal pickers (like AccountPicker) in SwiftUI, applying `.buttonStyle(.plain)` completely removes their interactive context. While `.accessibilityLabel` is necessary, dynamically including the `[.isSelected, .isButton]` traits is critical for VoiceOver users to understand both the purpose (it's a button) and its current state (whether it is active/selected).
**Action:** Consistently combine `.accessibilityLabel`, `.accessibilityHint`, and conditional `.accessibilityAddTraits` based on the selected state whenever implementing custom segmented controls or pickers using `.plain` buttons.
