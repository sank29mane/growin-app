...
## 2024-05-24 - Provide Contextual Accessibility to TextFields
**Learning:** In SwiftUI, `TextField` elements rely on their placeholder values to be read by VoiceOver by default. When the placeholder is generic (like "Amount" or "0.00"), it lacks context. Adding an explicit `.accessibilityLabel` (for the identity of the field) and `.accessibilityHint` (for usage instruction) provides a significantly improved experience for screen reader users compared to the default behavior.
**Action:** When adding or modifying a `TextField`, always consider if the placeholder provides enough context on its own. If not, immediately append `.accessibilityLabel` and `.accessibilityHint` to clarify the input's purpose.
