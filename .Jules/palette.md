...
## 2026-07-20 - Optional Accessibility Hints for Reusable Components
**Learning:** In SwiftUI, generic reusable interactive components (like `PremiumButton` in `ThemeComponents.swift`) inherently lack the context of their specific consequence at the call site. Standard buttons initialized with text automatically receive `.isButton` trait and use the text string as their default `.accessibilityLabel`. Explicitly adding `.accessibilityLabel` or `.accessibilityAddTraits(.isButton)` to them is redundant and considered an accessibility anti-pattern. However, allowing call sites to optionally add `.accessibilityHint` is good practice so screen reader users understand consequence context for actions like "BOOT ENGINE" or "Analyze".
**Action:** Always include an optional `accessibilityHint` (or `accessibilityHintText`) parameter applied via a conditional ViewModifier (or directly on the view) to allow call sites to provide VoiceOver users with concrete consequence context, while removing redundant accessibility modifiers.
...
## 2026-07-28 - TextEditor Accessibility Context
**Learning:** In SwiftUI, `TextEditor` elements without explicit accessibility modifiers lack context for VoiceOver users, unlike standard text inputs which sometimes infer context. VoiceOver may just announce "text field" without explaining what the user is supposed to type, causing confusion in complex forms like the Challenge Logic view.
**Action:** Always append explicit `.accessibilityLabel` and `.accessibilityHint` modifiers to `TextEditor` elements to provide clear context (e.g., `.accessibilityLabel("Challenge Logic Details")`, `.accessibilityHint("Enter your reasoning...")`) for assistive technologies.
