...
## 2026-03-31 - Missing Accessibility in Account Pickers and Dynamic Chips
**Learning:** Found multiple dynamic chip interfaces in Chat views (`AccountPicker`, `WelcomeView`'s `SuggestionChip`) using `.buttonStyle(.plain)` which stripped crucial element metadata. In grouping elements like chips that merge icons and text, screen readers fail to synthesize them cleanly without manual combining.
**Action:** When adding `.plain` to composite button components, always combine children using `.accessibilityElement(children: .combine)`, dynamically map the `.isSelected` trait for toggles, and use explicit hints for specialized actions (e.g., "Fills chat input with...").
