...
**Action:** Always append an explicit `.accessibilityHint` (e.g., "Navigates to the \(label) view") alongside `.accessibilityLabel` and `.accessibilityAddTraits(.isButton)` when applying `.buttonStyle(.plain)` to tab or sidebar navigation elements, providing clear consequence context for assistive technologies.
