...
## 2024-07-26 - NSCursor Balancing on Disappear
**Learning:** When balancing NSCursor.push() with NSCursor.pop() inside .onHover handlers in macOS native SwiftUI views, the element might disappear while still being hovered (e.g., when navigating to a new view). This bypasses the isHovered == false condition, resulting in a permanently stuck cursor.
**Action:** Always add an .onDisappear modifier that conditionally calls NSCursor.pop() if the view was hovered to ensure the cursor is properly restored.
