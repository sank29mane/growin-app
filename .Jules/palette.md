...
## 2026-06-25 - SecureField Placeholder Accessibility
**Learning:** In SwiftUI, `SecureField` elements that rely on generic placeholders (e.g., `SecureField("sk-...", text: $openaiApiKey)`) are read by VoiceOver merely as the placeholder string. Even if they are visually grouped with descriptive text elements above them (like "OpenAI API Key"), screen readers treat those as disconnected entities, resulting in a loss of context, similarly to `TextField`.
**Action:** Always append an explicit `.accessibilityLabel` and `.accessibilityHint` directly to `SecureField` elements to provide clear context (e.g., `.accessibilityLabel("OpenAI API Key Input")`, `.accessibilityHint("Enter your OpenAI API key")`) for assistive technologies.
