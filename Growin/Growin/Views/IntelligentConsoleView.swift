...
struct IntelligentConsoleView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        HStack(spacing: 12) {
                            Text(systemStatus?.status?.uppercased() ?? "OFFLINE")
                                ...
                                .foregroundColor(systemStatus?.status?.lowercased() == "healthy" ? .green : .red)
                ...
                // Desktop Layout: Metrics + Agent Status Side-by-Side
    ...
    private func fetchLogs() {
        guard let url = URL(string: "\(baseURL)/debug/logs") else { return }
